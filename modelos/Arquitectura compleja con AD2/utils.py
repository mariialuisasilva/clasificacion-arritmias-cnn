...
NIVEL_INTENSIDAD_AD = 0.05  # Rango de intensidad para los canales de color RGB para la data
                            # augmentation

def augment_image(image, seed_value):
    # Establecer una semilla para las operaciones aleatorias
    #tf.random.set_seed(seed_value)
    
    # Añadir ruido gaussiano
    # Aplica diferentes cambios en cada canal y en cada píxel de la imagen
    ...
    seed_value = tf.constant([seed_value, 0], dtype=tf.int32)

    image = tf.image.stateless_random_contrast(image, lower=0.8, upper=1.2, seed=seed_value)
    image = tf.image.stateless_random_saturation(image, lower=0.8, upper=1.2, seed=seed_value)

    # Recortar los valores de la imagen para que estén dentro del rango [0, 255]
    image = tf.clip_by_value(image, 0.0, 255.0)

    # Asegura el tamaño final si hay recortes previos en algún pipeline
